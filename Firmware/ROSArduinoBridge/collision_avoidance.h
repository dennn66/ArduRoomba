...
void printDistances(){
  String space=" ";
  DEBUG_UART.print(frontDistance[0].getMedian());
  DEBUG_UART.print(space);
  DEBUG_UART.print(frontDistance[1].getMedian());
  DEBUG_UART.print(space);
  DEBUG_UART.println(frontDistance[2].getMedian());
}
...
void stop(){
  if (ENABLE_DEBUG) DEBUG_UART.println("S");
  motorCommand(0,0);
...
void turn(boolean isLeft){
  if (isLeft){
      //turn left
      if (ENABLE_DEBUG) DEBUG_UART.println("L");
      motorCommand(-TURN_SPEED, TURN_SPEED);
  }else{
      //turn right
      if (ENABLE_DEBUG) DEBUG_UART.println("R");
      motorCommand(TURN_SPEED, -TURN_SPEED);
  }
...
void moveForward(){
  if (ENABLE_DEBUG) DEBUG_UART.println("F");
...
void moveBackward(){
  if (ENABLE_DEBUG) DEBUG_UART.println("B");
...
boolean isStuck(){
  ...
  if (stuckCounterBucket > STOP_MAX_STUCK_BUCKET_COUNT){
    ...
    if (ENABLE_DEBUG) DEBUG_UART.println("ES");
    return true;
  }
  ...
  /*if (abs(readEncoder(0) - leftPID.prevEnc) < MIN_TICKS || abs(readEncoder(1) - rightPID.prevEnc) < MIN_TICKS){
    stuckCounterBucket++;
    if (ENABLE_DEBUG) DEBUG_UART.println("EI");
  } else if (stuckCounterBucket > 0) {
    stuckCounterBucket--; //decrement bucket
    if (ENABLE_DEBUG) DEBUG_UART.println("ED");
  }*/
  ...
  if (ENABLE_DEBUG) {
    DEBUG_UART.print("EC ");
    DEBUG_UART.println(stuckCounterBucket);
  }
...
/* Escape routine */
void escape(){
  if (ENABLE_DEBUG) DEBUG_UART.println("E");
...
void ca_init(){
    ...
    if (ENABLE_DEBUG) DEBUG_UART.println("Collision Setup Ready");
}
