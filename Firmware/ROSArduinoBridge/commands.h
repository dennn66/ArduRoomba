...
int runCommand() {
  ...
  switch(cmd) {
  case GET_BAUDRATE:
    SL_UART.println(BAUDRATE);
    break;
  case ANALOG_READ:
    SL_UART.println(analogRead(arg1));
    break;
  case DIGITAL_READ:
    SL_UART.println(digitalRead(arg1));
    break;
  case ANALOG_WRITE:
  ...
  case SERVO_WRITE:
    ...
    break;
  case SERVO_READ:
    SL_UART.println(servos[arg1].read());
    break;
  case ALL_SERVO_READ:
    for (i = 0; i < N_SERVOS; i++) {
       SL_UART.print(servos[i].read());
       SL_UART.print(" ");
    }
    SL_UART.println("");
    break;
  case ALL_SERVO_STATE:
    for (i = 0; i < N_SERVOS; i++) {
       SL_UART.print(servos[i].attached());
       SL_UART.print(" ");
    }
    SL_UART.println("");
    break;
  ...
  case MAGNETO_READ:
    SL_UART.print(readMilliGauss_OnThe_XAxis());
    SL_UART.print(" ");
    SL_UART.print(readMilliGauss_OnThe_YAxis());
    SL_UART.print(" ");
    SL_UART.println(readMilliGauss_OnThe_ZAxis());
    break;
#endif //MAGNETO_HMC5883L
  case READ_ENCODERS:
    SL_UART.print(readEncoder(LEFT));
    SL_UART.print(" ");
    SL_UART.println(readEncoder(RIGHT));
    break;
   case RESET_ENCODERS:
    resetEncoders();
...
#endif //USE_BASE
  default:
    SL_UART.println("Invalid Command");
    break;
  }
}
