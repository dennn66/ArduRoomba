...
long lastpingtime=0;
uint8_t lastsonar=0;
uint8_t sonars[NSONARS]={SONAR_FRONT_CENTER,SONAR_LEFT, SONAR_FRONT_LEFT, SONAR_FRONT_RIGHT, SONAR_RIGHT};
...
void driveDirect(int leftVel, int rightVel)
{
   ...
   if(oi.getSensorValue(OI_SENSOR_OI_MODE) == OI_MODE_SAFE ||
        ...
        int obsticle_cm = min(oi.getSensorValue(OI_WALL_SIGNAL), min(oi.getSensorValue(OI_CLIFF_FRONT_LEFT_SIGNAL), oi.getSensorValue(OI_CLIFF_FRONT_RIGHT_SIGNAL)));
...
void spinOI()
{
  ...
  int analoggyro = (int)((2.5+gyro.data.y*dps2mV)/5*1024);
...
//       driveDirect(-irVelocity, irVelocity);
  ...
  if(millis()-lastpingtime>PINGINTERVAL){
    ...
    oi.setSensorValue(OI_WALL_SIGNAL+lastsonar, sonar_cm);
    sonar_state = (sonar_cm < 6)?1:0;
    oi.setSensorValue(OI_WALL+lastsonar, sonar_state);
  }
#define RIGHT_BUMP_BIT 0
#define LEFT_BUMP_BIT  1
  uint8_t bumps_state=(uint8_t)oi.getSensorValue(OI_SENSOR_WHEELDROPS_BUMPS);
  if(oi.getSensorValue(OI_WALL)==1|oi.getSensorValue(OI_CLIFF_FRONT_LEFT)==1) {
    bumps_state |= (1<<LEFT_BUMP_BIT);
  } else {
    bumps_state &= ~(1<<LEFT_BUMP_BIT);
  }  
  if(oi.getSensorValue(OI_WALL)==1|oi.getSensorValue(OI_CLIFF_FRONT_RIGHT)==1) {
